...
/**
 * Scraper of an old layout of transparent account. It's still in use
 * (eg. Milos Zeman's 1st presidental election account www.csas.cz/banka/nav/o-nas/transparentni-ucet-23902000730800-d00018326)
 *
 * @author Michal Bernhard (michal@bernhard.cz) 2013
 */
class CeskaSporitelnaTransparentAccountOldInfoScraper implements AccountInfoScraper {
    ...
    def TransactionItem parseRow(Element row) {
        ...
        def amountElement = row.select("td")[4]
        def amountRawText = ""
        //handle <p> in <td>
        if (amountElement.html().contains("<p>")) {
            amountRawText = amountElement.select("p").html()
        } else {
            amountRawText = amountElement.text()
        }

        if (cz.zavodprezidentu.utils.StringUtils.isBlank(amountRawText)) {
            amountRawText = "0"
        }
